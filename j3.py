...
class J3:
    ...
    __isopen = False
    __handle = None
    __dll = cdll.LoadLibrary(path.join(path.dirname(path.abspath(__file__)), 'Fwlib64.dll'))
    ...
    def __open(self):
        ...
        if not self.__isopen:
            ...
            handle = c_ushort()
            res = self.__dll.cnc_allclibhndl3(bytes(self.__ip, 'utf-8'), c_ushort(int(self.__port)), c_long(10),
                                              byref(handle))
    ...
    # --- NC programme file operation related ---.

    def exist_file(self, path):
        """NC program search and return bool。
        ...
        filenm = path.split('/')[-1]
        if filenm[0:1] != 'O':
            raise Exception('仅可检索加工PG（开头为O的编号）。')
        with self.__lock:
            self.__open()
    ...
    def read_file(self, path):
        ...
        with self.__lock:
            ...
            if not self.exist_file(path):
                raise Exception('The specified machining program does not exist。(path: ' + str(path) + ')')

            # EW_DATA(5): データの誤り の詳細エラー内容
            ew_data_errmap = {
                2: 'No program is registered within the specified range.',
                3: 'The NC program area is broken.'}
            ...
            try:
                # 要求CNC侧开始读取NC程序
                data_type = c_short(0)  # 0: NC command program
                file_name_p = c_char_p(create_string_buffer(bytes(path, 'utf-8')).raw)  # File name to read
                ...
                while True:
                    length_p = c_long(1024)  # Pointer sets the number of characters actually output
    ...
    def write_file(self, path, data):
        ...
        with self.__lock:
            ...
            try:
                ...
                dir_name_p = c_char_p(
                    create_string_buffer(bytes('//CNC_MEM/USER/LIBRARY/', 'utf-8')).raw)  # Writeするディレクトリ名
    ...
    def find_dir(self, path):
        ...
        with self.__lock:
            ...
            self.__dll.cnc_rdpdf_alldir.argtypes = (
                c_ushort, POINTER(c_short), POINTER(J3.IDBPDFADIR), POINTER(J3.ODBPDFADIR))
            while True:
                res = self.__dll.cnc_rdpdf_alldir(self.__handle, byref(num_prog_p), byref(pdf_adir_in),
                                                  byref(pdf_adir_out))
                self.__cnc_raise_error(res)
    ...
    def read_dev(self, dev, size=1):
        ...
        with self.__lock:
            ...
            self.__dll.pmc_rdpmcrng.argtypes = (
                c_ushort, c_short, c_short, c_ushort, c_ushort, c_ushort, POINTER(J3.IODBPMC))
